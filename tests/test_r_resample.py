...
def test_replacement_true_regression_fits_and_predicts():
    ...
    RLT_reg(n_estimators=30, resample_replace=True, n_jobs=2, verbose=0).fit(
        d["X"], d["y"]
    )
...
def test_replacement_true_classification_fits_and_predicts():
    ...
    RLT_cla(n_estimators=30, resample_replace=True, n_jobs=2, verbose=0).fit(
        d["X"], d["y"]
    )
...
def test_replacement_true_survival_fits_and_predicts():
    ...
    RLT_surv(n_estimators=30, resample_replace=True, n_jobs=2, verbose=0).fit(
        d["X"], d["y"]
    )
...
def test_replacement_true_stored_in_parameters():
    d = generate_simple_regression(n=80, p=10)
    fit = RLT_reg(n_estimators=30, resample_replace=True, n_jobs=2, verbose=0).fit(
        d["X"], d["y"]
    )
...
def test_replacement_false_regression_fits_and_predicts():
    d = generate_simple_regression(n=80, p=10)
    RLT_reg(n_estimators=30, resample_replace=False, n_jobs=2, verbose=0).fit(
        d["X"], d["y"]
    )
...
def test_replacement_false_classification_fits_and_predicts():
    d = generate_classification_data(n=80, p=10)
    RLT_cla(n_estimators=30, resample_replace=False, n_jobs=2, verbose=0).fit(
        d["X"], d["y"]
    )
...
def test_replacement_false_survival_fits_and_predicts():
    d = generate_survival_data(n=80, p=10, seed=1)
    RLT_surv(n_estimators=30, resample_replace=False, n_jobs=2, verbose=0).fit(
        d["X"], d["y"]
    )
...
def test_replacement_false_stored_in_parameters():
    d = generate_simple_regression(n=80, p=10)
    fit = RLT_reg(n_estimators=30, resample_replace=False, n_jobs=2, verbose=0).fit(
        d["X"], d["y"]
    )
...
def test_resample_prob_050_regression_subsampling_works():
    d = generate_simple_regression(n=80, p=10)
    RLT_reg(
        n_estimators=30, resample_prob=0.5, resample_replace=False, n_jobs=2, verbose=0
    ).fit(d["X"], d["y"])
...
def test_resample_prob_080_regression_subsampling_works():
    d = generate_simple_regression(n=80, p=10)
    RLT_reg(
        n_estimators=30, resample_prob=0.8, resample_replace=False, n_jobs=2, verbose=0
    ).fit(d["X"], d["y"])
...
def test_resample_prob_0632_regression_bootstrap_works():
    d = generate_simple_regression(n=80, p=10)
    RLT_reg(
        n_estimators=30, resample_prob=0.632, resample_replace=True, n_jobs=2, verbose=0
    ).fit(d["X"], d["y"])
...
def test_bootstrap_produces_oob_predictions():
    d = generate_simple_regression(n=80, p=10)
    fit = RLT_reg(
        n_estimators=30, resample_replace=True, resample_prob=1.0, n_jobs=2, verbose=0
    ).fit(d["X"], d["y"])
...
def test_subsampling_produces_oob_predictions():
    d = generate_simple_regression(n=80, p=10)
    fit = RLT_reg(
        n_estimators=30, resample_replace=False, resample_prob=0.5, n_jobs=2, verbose=0
    ).fit(d["X"], d["y"])
    assert np.asarray(fit.oob_prediction_).shape == (d["n"],)
